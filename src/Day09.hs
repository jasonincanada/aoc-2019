...
data Value   = Value Int    -- interpretable as an opcode
...
initComp :: IM.IntMap Value -> [Value] -> Computer
initComp program inputs = Computer (Addr 0)
                                   program
                                   0
                                   Position
                                   Position
...
update (Addr a) v = do
  memory' <- IM.insert a v <$> gets memory
